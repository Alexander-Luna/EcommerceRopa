...
columns: [
      { data: "color", title: "Nombre" },
      {
        ...
        render: function (data, type, row) {
          // Renderiza el color hexadecimal como un círculo de color y lo muestra junto con el valor en formato string
          return (
  ...
  $(document).on("click", ".btnEditar", function () {
    ...
    $("#miModal").modal("show");
    document.getElementById("title").innerText = "Editar Color";
    $("#id").val(rowData.id);
    $("#nombre").val(rowData.color);
    $("#color_hexa").val(rowData.color_hexa);
    $("#est").val(rowData.est);
  });
  ...
  function insertar() {
    try {
      ...
      formData.append("color", nombre);
      formData.append("color_hexa", color_hexa);
      formData.append("est", est);
      if (id === "") {
        fetch("../../controllers/router.php?op=insertColor", {
          method: "POST",
          body: formData,
        })
          .then((response) => {
        ...
        fetch("../../controllers/router.php?op=updateColor", {
          method: "POST",
          body: formData,
        })
          .then((response) => {
  ...
  function reloadSection() {
    try {
      fetch("../../controllers/router.php?op=getAllColor").then((response) => {
        if (!response.ok) {
          throw new Error(
            "Hubo un problema al obtener los detalles del color."
          );
        }
